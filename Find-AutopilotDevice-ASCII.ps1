...
.EXAMPLE
    .\Find-AutopilotDevice-ASCII.ps1 -DisplayName "PC-NAME"
    Search for devices with "PC-NAME" in the display name

.EXAMPLE
    .\Find-AutopilotDevice-ASCII.ps1 -DeviceId "12345678-1234-1234-1234-123456789abc"
    Search for device by Entra Device ID

.EXAMPLE
    .\Find-AutopilotDevice-ASCII.ps1 -DisplayName "PC-NAME" -WhatIf
...
if ($script:SearchTarget -eq "Entra") {
    ...
    foreach ($device in $uniqueDevices) {
        ...
        try {
            ...
            Write-ColorOutput "[OK] Successfully deleted from Entra ID" "Green"
        } catch {
            Write-ColorOutput "[X] Error: $($_.Exception.Message)" "Red"
        }
    }
    ...
    foreach ($device in $uniqueDevices) {
        ...
        try {
            ...
            Invoke-MgGraphRequest -Uri $deleteUri -Method DELETE
            Write-ColorOutput "[OK] Successfully queued for deletion" "Green"
        } catch {
            Write-ColorOutput "[X] Error: $($_.Exception.Message)" "Red"
        }
    }
