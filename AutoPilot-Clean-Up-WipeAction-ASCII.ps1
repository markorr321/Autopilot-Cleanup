...
.NOTES
    Author: Mark Orr
    Date: December 2024
    Version: ASCII-compatible (no Unicode characters)
#>
...
function Connect-ToGraph {
    ...
    try {
        ...
        Write-ColorOutput "[OK] Connected to Microsoft Graph" "Green"
        return $true
    }
    catch {
        Write-ColorOutput "[X] Failed to connect: $($_.Exception.Message)" "Red"
        return $false
    }
}
...
function Wait-ForWipeCompletion {
    ...
    while ($true) {
        ...
        if ($elapsed -ge $timeoutSeconds) {
            Write-ColorOutput "[X] TIMEOUT after $TimeoutMinutes minutes" "Red"
            return $false
        }
        ...
        if ($null -eq $device) {
            Write-ColorOutput "[$timestamp] [OK] Device removed from Intune (wipe complete)" "Green"
            return $true
        }
...
function Remove-AutopilotDeviceRecord {
    ...
    try {
        ...
        Write-ColorOutput "  [OK] Autopilot deletion initiated" "Green"
        return @{ Success = $true; Found = $true; Error = $null }
    }
    catch {
        $errorMsg = $_.Exception.Message
        if ($errorMsg -match "NotFound|404") {
            Write-ColorOutput "  [OK] Autopilot (already removed)" "Green"
        ...
        Write-ColorOutput "  [X] Autopilot: $errorMsg" "Red"
...
function Wait-ForAutopilotRemoval {
    ...
    do {
        ...
        if (-not $device) {
            Write-ColorOutput "  [OK] Autopilot removal verified ($elapsedMinutes min)" "Green"
            return $true
        }
...
function Remove-EntraDeviceRecords {
    ...
    Write-ColorOutput "  [OK] Entra ID ($count device(s))" "Green"
    return $true
}
...
Write-ColorOutput ""
if ($SkipWipe) {
    Write-ColorOutput "[!] This will REMOVE records (wipe skipped)" "Yellow"
} else {
    Write-ColorOutput "[!] WARNING: This will WIPE device(s) and REMOVE all records!" "Red"
}
Write-ColorOutput ""
...
foreach ($device in $targetDevices) {
    ...
    Write-ColorOutput "  [OK] DEVICE CLEANUP COMPLETE: $($device.DeviceName)" "Green"
    Write-ColorOutput "============================================================" "Green"
}
