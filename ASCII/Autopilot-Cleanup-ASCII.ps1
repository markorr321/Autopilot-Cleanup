...
.NOTES
    ...
    Version: 2.0-ASCII (no Unicode characters)
#>
...
function Connect-ToGraph {
    ...
    try {
        ...
        Write-ColorOutput "[OK] Successfully connected to Microsoft Graph" "Green"
        return $true
    }
    catch {
...
function Wait-ForDeviceWipe {
    ...
    while ($true) {
        ...
        if ($elapsed -ge $timeoutSeconds) {
            Write-ColorOutput "[X] TIMEOUT - Wipe did not complete within $TimeoutMinutes minutes" "Red"
            return $false
        }
...
switch ($actionChoice) {
    ...
    "2" {
        ...
        Write-ColorOutput ""
        Write-ColorOutput "[!] WARNING: This will FACTORY RESET the selected device(s)!" "Red"
...
foreach ($selectedDevice in $selectedDevices) {
        ...
        if ($intuneDevice) {
            ...
            if ($wipeResult) {
                ...
                if ($script:NoLoggingMode) {
                    Write-ColorOutput "[OK] Device processed for wipe (no status check)" "Cyan"
    ...
    if ($script:NoLoggingMode) {
        ...
        Write-ColorOutput ""
        Write-ColorOutput "============================================================" "Cyan"
        Write-ColorOutput "  [OK] DEVICE PROCESSED FOR REMOVAL" "Cyan"
        Write-ColorOutput "============================================================" "Cyan"
